// https://github.com/DLily0129/Data-Structure/blob/master/OpenHash.cpp
//  because the hashtable is a link node hash table
//  so maybe one Node has so many data 
//  so call the open hash
#include <iostream>
using namespace std;
...
void OpenHashTable::Insert(int key)
{
	...
	else// in fact, function Find return the last point of Hash[i], so p must be NULL
	    cout<<"该键值已存在"<<endl;
}
